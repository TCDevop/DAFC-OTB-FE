...
const UnsavedChangesBanner = ({
  ...
  onSaveDraft,
  onDiscard,
  saving = false}: UnsavedChangesBannerProps) => {
  const { t } = useLanguage();
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-medium ${'text-[#6B4D30]'}`}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onSaveDraft}
            disabled={saving}
            className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-semibold transition-colors ${'bg-[rgba(18,119,73,0.12)] text-[#127749] hover:bg-[rgba(18,119,73,0.2)]'}`}
          >
            {saving ? (
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div
            className={`w-full max-w-sm mx-4 rounded-xl border shadow-2xl p-5 ${'bg-white border-[#C4B5A5]'}`}
          >
            <div className="flex items-center gap-2 mb-3">
              <AlertTriangle size={18} className="text-[#F85149]" />
              <h3
                className={`font-semibold font-['Montserrat'] ${'text-[#0A0A0A]'}`}
              >
